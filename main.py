...
(P.S. Не добавлять сайт - /cancel)""")
            ...
            url = requests.get(f'{address}',
                               headers={'User-Agent': 'Mozilla/5.0'})
            # response = urlopen(url)
            # last_modified = response.links
            # res = dict()
            website.version = hashlib.sha224(url.content).hexdigest()
            match address.split():
...
async def check_site_on_upd(context):
    ...
    #     url = requests.get(f'{site.address}',
    #                   headers={'User-Agent': 'Mozilla/5.0'})
    #     # response = urlopen(url)
    #     # last_modified = response.links
    #     # res = dict()
    #     current_hash = hashlib.sha224(url.content).hexdigest()
...
def main():
    ...
    job = app.job_queue
    job.run_repeating(check_site_on_upd, interval=5)
    app.run_polling()
